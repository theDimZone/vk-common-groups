...
* TODO: fix inputs; it breaks when count of inputs >= 8
...
* [`src/vk_common_groups/styles.cljs`](src/clj/vk_common_groups/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/vk_common_groups/`](src/vk_common_groups/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/vk_common_groups/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `vk-common-groups` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/vk_common_groups/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns vk-common-groups.example
  (:require [vk-common-groups.config :as config])
